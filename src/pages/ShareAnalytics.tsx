...
import { TrendingUp, TrendingDown, BarChart2, X, Search, FileText, ChevronDown, ChevronUp, Activity, BookOpen, DollarSign, Plus, Pencil, Trash2, Save } from 'lucide-react';
...
// ── Shared helpers for analytics tabs ────────────────────────────────────────
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {['Share','Effective Date','52-Week High','52-Week Low','Range','Notes',''].map(h => (
                  <th key={h} className={`px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide ${h === 'Share' || h === 'Effective Date' || h === 'Notes' || h === '' ? 'text-left' : 'text-right'}`}>{h}</th>
                ))}
              </tr>
            </thead>
...
// ── Share Values Tab (Net Book Value) ─────────────────────────────────────────
...
type AnalyticsTab = 'portfolio' | '52week' | 'earnings' | 'share-values' | 'dps';
...
export function ShareAnalytics() {
  ...
  const TABS: { id: AnalyticsTab; label: string; icon: React.ReactNode }[] = [
    { id: 'portfolio',     label: 'Portfolio Analytics', icon: <BarChart2 className="w-4 h-4" /> },
